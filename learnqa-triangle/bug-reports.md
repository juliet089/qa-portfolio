...
"Это равносторонний треугольник. Вы ввели: A: 0; B: 0; C: 0"
...
## Баг #5: Форма ввода значений сторон треугольника не валидирует поле C
...
**Шаги воспроизведения:**
1. Ввести любые целые числа в поля значений A и B
2. Поле C оставить пустым
3. Нажать кнопку "ПОКАЗАТЬ"

**Фактический результат:** Программа ведёт себя так, будто в поле C есть какое-то некорректное значение и выдаёт сообщение об ошибке "Это НЕ треугольник."

**Ожидаемый результат:** Появляется сообщение об ошибке, аналогичное реакции системы на другие пустые поля ("Задайте все стороны.")

**Severity:** Minor — Дефект не затрагивает основную функциональность, но может ухудшать пользовательский опыт

**Priority:** Low — Дефект не затрагивает основной функционал и не мешает пользоваться сервисом

**Вложения:**

![Отсутствие значения в поле C](./screenshots/bug5-no-val-c.png)
